...
def convert(checkpoint, model_path, model_cfg, safe_serialization=True):
    ...
    tmp_model_path = os.path.join(model_path, "llama")
    ...
    for l in range(int(num_layers)):
        logging.info(f"start converting layer {l}")
        state_dict = {}
        ...
        logging.info(f"finish converting layer {l}")
    ...
    logging.info(f"Loading the checkpoint in a LLaMA model.")
    model = LlamaForCausalLM.from_pretrained(tmp_model_path, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True)
    # Avoid saving this as part of the config.
    del model.config._name_or_path
    model.config.torch_dtype = torch.bfloat16
    logging.info(f"Saving in the Transformers format.")
...
def main(cfg) -> None:
    ...
    if cfg.convert.input_nemo_ckpt is not None and cfg.convert.output_hf_ckpt is not None:
        checkpoint = MegatronGPTModel.load_from_checkpoint(checkpoint_path=cfg.convert.input_nemo_ckpt, trainer=trainer, convert_ckpt=True)
        convert(checkpoint, cfg.convert.output_hf_ckpt, cfg.model)
        logging.info(f"Conversion finished: from nemo-megatron ckpt to huggingface ckpt")
        logging.info(f"Converted huggingface ckpt path: {cfg.convert.output_hf_ckpt}")
        return
