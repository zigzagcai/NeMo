    ...
    def load_checkpoint(self, checkpoint_path: Union[str, Path]) -> Dict[str, Any]:
        def write_json(text, path):
            with open(path, "w") as f:
                json.dump(text, f)
        def convert(checkpoint, model_path="/mnt/petrelfs/caizheng/hf_model_ckpt", safe_serialization=True):
            os.makedirs(model_path, exist_ok=True)
            tmp_model_path = os.path.join(model_path, "llama-2-1b-nemo")
            ...
            logging.info(f"Finished check model loading.")
